<template>
      ...
      <el-container>
        <el-container>
          <el-aside width="300px">
            ...
            <!-- <el-button type="primary" @click="treeEdit">编辑</el-button> -->
            ...
            <el-popconfirm title="确认删除吗" confirm-button-text="是" cancel-button-text="否" @confirm="treeDelete">
              <template #reference>
                <el-button class="headerButton" type="primary" plain>删除</el-button>
              </template>
            </el-popconfirm>
            <el-tree highlight-current :data="treeData" :props="defaultProps" @node-click="treeNodeClick"></el-tree>
            ...
            <el-table :data="tableData" stripe max-height="680">
              ...
              <el-table-column label="操作列" width="200" fixed="right">
                <template #default="scope">
                  <!-- <el-button link type="primary" size="small" @click.prevent="onDownload(scope.row.id, scope.row.kb_file_name)">下载</el-button> -->
                  <el-button class="headerButton" type="primary" plain size="small" @click.prevent="onEdit(scope.row)">编辑</el-button>
                  <!-- <el-button link type="primary" size="small" @click.prevent="FaqonDelete(scope.row.id)">删除</el-button> -->
 ...
 <el-dialog v-model="treeDialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" title="组名称" width="500">
    <el-form :model="treeFormData" label-position="left" label-width="80px">
      ...
      <el-form-item>
        <el-button class="headerButton" type="primary" plain @click="treeCancel">取消</el-button>
        <el-button class="headerButton" type="primary" @click="treeSubmit">提交</el-button>
      </el-form-item>
    </el-form>
  </el-dialog>
...
onMounted(async () => {
  ...
  // 初始化文件列表
  const Res = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqFileUploadIList({ pageEnable: false });
...
async function handleQuery(initLoad = false) {
  try {
    ...
    const res = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqInfoList(params);
...
async function treeList() {
  const res = await  KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqGroupInfoList({
    pageEnable: false
  })
  if (res.status === 200) {
...
async function treeSubmit() {
  try {
    ...
    if (!formData.value.id) {
      const checkRes = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqGroupInfoList({
        pageEnable: false,
        pageRule: [{
          field: "group_key",
          rule: "is",
    ...
    const res = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqGroupInfo(formData.value);
...
async function treeDelete() {
  ...
  // 检查节点下是否有子记录
  // 获取节点下的子记录总数
  const res = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqInfoList({
    pageEnable: false,
    pageRule: [{
      field: "group_id", 
      rule: "is",
  ...
  try {
    const res = await KnowledgeBaseFaqFileUpload.DeleteKnowledgeBaseFaqGroupInfo({ id: id })
    ElMessage({
...
async function onAdd() {
  ...
  try {
    // 尝试获取文件列表，但不阻塞对话框打开
    // const res = await KnowledgeBaseFaqFileUploadList({ 
    //   pageEnable: false,
    //   pageRule: [{field: "group_id", rule: "is", value: activeTreeNode.value.id}]
    // });
    // File_Name_Option.value = (res.data?.data?.data || []).map(item => ({
    //   id: item.id.toString(),
    //   file_name: item.file_name
    // }));
    formData.value = {}
    const Res = await KnowledgeBaseFaqFileUpload.PostKnowledgeBaseFaqFileUploadIList({
      pageEnable: true,
  ...
  formData.value = {
    ...
    kb_file_name: formData.value.kb_file || '无文件' // 使用正确的字段名
  };
  ...
  const res = await KnowledgeBaseFaqInfoPost(formData.value)
  ElMessage({
    message: res.data.message,
    type: res.data.code,
    plain: true,
...
async function onDelete(id) {
  try {
    const res = await  KnowledgeBaseFaqInfoDelete({ id: id })
    ElMessage({
      message: res?.data?.message || '删除成功',
      type: res?.data?.code === 200 ? 'success' : 'error',
      plain: true,
...
async function onEdit(row) {
  try {
    ...
    formData.value = {
      ...
      kb_file: rowData.kb_file_name, // 使用正确的字段名
...
//   try {
//     const response = await fetch(`${BASE_URL}/knowledge_base_faq/download?id=${id}`);
