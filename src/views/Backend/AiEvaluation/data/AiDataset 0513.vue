    ...
    <div class="left-panel">
      ...
      <div class="tree-header">
        <el-aside width="300px">
          <el-button type="primary" plain @click="treeAdd({ level: 0 })">新增节点</el-button>
          <el-button type="primary" plain @click="treeEdit({ level: 0 })">编辑节点</el-button>
          <el-button type="danger"  plain @click="selectedNode.value && selectedNode.value.id ? treeDelete(selectedNode.value, null) : ElNotification.warning('请选中要删除的节点')" >删除节点</el-button>
        </el-aside>
      </div>
      <div class="tree-container">
        ...
        >
          <template #default="{ node, data }">
            ...
            >
              {{ data.name }}
              <el-button type="danger" size="small" plain style="margin-left: 8px;" @click.stop="treeDelete(data, node)">删除</el-button>
            </span>
          </template>
        </el-tree>
      </div>
    ...
    <el-dialog v-model="addDialogVisible" title="新增节点">
      <el-form :model="addForm">
        ...
        <el-form-item label="父节点" required>
          <el-select v-model="addForm.parent" placeholder="请选择父节点">
            <el-option
...
// 树形结构新增
async function treeAdd(parentNode) {
...
// 处理删除节点
async function treeDelete(data, node) {
  if (!data) {
...
// 通用查找 name->id 的函数
async function getNameToIdMap(apiFunc, nameField = 'name', idField = 'id') {
  ...
  if (Array.isArray(res.data?.data?.data)) {
    ...
    console.error('接口返回值不是数组，实际为:', res)
    return {}
  }
  const map = {}
  arr.forEach(item => {
...
async function MultFileonSubmit() {
  ...
  const dataList = await parseExcel(file)
  console.log('dataList:', dataList)

  if (!Array.isArray(dataList)) {
    ElNotification({ message: 'Excel解析失败，内容不是数组', type: 'error' })
    return
  }

  // 获取 name->id 映射
  const { tagMap, entityMap, datasetMap } = await prepareNameToIdMaps()

  console.log('entityMap所有key:', Object.keys(entityMap))
  console.log('datasetMap所有key:', Object.keys(datasetMap))
  console.log('tagMap所有key:', Object.keys(tagMap))

  // 逐条导入
  const [header, ...rows] = dataList
  // rows 现在只包含数据行，不包含表头
  const objList = rows
    .filter(rowArr => rowArr && rowArr.length && rowArr.some(cell => cell !== undefined && cell !== null && cell !== '')) // 过滤空行
    .map(rowArr => {
      const obj = {}
  ...
  for (const row of objList) {
    // 这里 row 就是每一条实际数据，不会有表头
    // 组装 item 并插入
    const item = {
      question: row['标准问题'] != null ? row['标准问题'].toString() : '',
      ground_truth: row['标准答案'] != null ? row['标准答案'].toString() : '',
      contexts: row['上下文'] != null ? row['上下文'].toString() : '',
      answer: row['测评答案'] != null ? row['测评答案'].toString() : '',
      update_time: dayjs(row['更新时间']).format('YYYY-MM-DD HH:mm:ss'),
      // entity_id: entityMap[normalize(row['测试对象'])],
      // tag_id: tagMap[normalize((row['问题标签'] || '').trim())],
      // dataset_id: datasetMap[normalize(row['数据集'])]( || '').trim(

      entity_id: row['测试对象'] ? row['测试对象'] : DEFAULT_ENTITY_ID,
      tag_id: (row['问题标签'] || '').trim() ? (row['问题标签'] || '').trim() : DEFAULT_TAG_ID,
      dataset_id: normalize(row['数据集']) ? normalize(row['数据集']) : DEFAULT_DATASET_ID
    }
    // 校验必填项
    if (!item.question || !item.entity_id || !item.tag_id || !item.dataset_id) {
      console.error('缺少必填项，跳过:', item)
      continue
    }
    console.log('tagMap:', tagMap)
    console.log('row[问题标签]:', row['问题标签'])
    console.log('tag_id:', item.tag_id)
    await AiEvaluation.postDatasetItemInfo(item)
  }

  // ElNotification({ message: '导入成功', type: 'success' })
  ElNotification({ message: '发生错误', type: 'error' })
  excelDialogVisible.value = false
  await getDatasetItemList() // 关键：刷新formInline列表
}

async function prepareNameToIdMaps() {
  // 并行获取三个映射对象
  const [tagMap, entityMap, datasetMap] = await Promise.all([
    getNameToIdMap(AiEvaluation.postDatasetTagList),
    getNameToIdMap(AiEvaluation.postDatasetItemList),
    getNameToIdMap(AiEvaluation.postFileList)
  ])
  // 打印调试
  console.log('1-tagMap:', tagMap)
  console.log('2-entityMap:', entityMap)
  console.log('3-datasetMap:', datasetMap)
  // 返回对象，方便后续解构
  return { tagMap, entityMap, datasetMap }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      if (jsonData.length > 1) {
        ...
        const updateTimeIdx = header.indexOf('更新时间')
        if (updateTimeIdx !== -1) {
          for (let i = 1; i < jsonData.length; i++) {
            const cell = jsonData[i][updateTimeIdx]
            if (cell) {
              // 处理 Excel 日期数字或字符串
              let formatted
              if (typeof cell === 'number') {
                ...
                if (jsDate) {
                  formatted = dayjs(
                    new Date(jsDate.y, jsDate.m - 1, jsDate.d, jsDate.H, jsDate.M, jsDate.S)
                  ).format('YYYY-MM-DD HH:mm:ss')
                }
              } else {
                // 普通字符串直接格式化
                formatted = dayjs(cell).format('YYYY-MM-DD HH:mm:ss')
              }
              jsonData[i][updateTimeIdx] = formatted || cell
            }
          }
        }
      }
      ...
      if (Array.isArray(jsonData) && jsonData.length > 0) {
        const header = jsonData[0]
        console.log('Excel表头:', header)
        for (let i = 1; i < jsonData.length; i++) {
          console.log(`第${i}行:`, jsonData[i])
        }
      }
      resolve(jsonData)
    }
...
function handleAdd() {
  // 打开弹窗时，父节点默认选中当前节点
  addForm.value.parent = selectedNode.value ? selectedNode.value.id : null
  addDialogVisible.value = true
}

async function submitAdd() {
  // 这里假设你有 addTreeNode API
  await addTreeNode({
    name: addForm.value.name,
    parent: addForm.value.parent
  })
  addDialogVisible.value = false
  await refreshTree() // 重新拉取树数据
  // 展开父节点
  currentNodeKey.value = addForm.value.parent
}
...
const DEFAULT_ENTITY_ID = 1   // 你可以根据实际情况设定
