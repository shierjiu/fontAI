<template>
  <div class="dataset-container">
    ...
    <el-table-column prop="create_Time" label="评测时间" width="250" class-name="ellipsis-cell">
      <template #default="scope">
        <el-input v-if="scope.row.editing" v-model="scope.row.editData.create_Time" placeholder="请输入"/>
        <span v-else class="ellipsis-cell" :title="scope.row.create_Time">{{ scope.row.createTime }}</span>
      </template>
    </el-table-column>
    ...
    <el-dialog v-model="detailDialogVisible" title="详情数据" width="40%">
      <el-table :data="Object.entries(detailData)">
...
async function getRecordList() {
  ...
  console.log('获得评估的记录:', recordList.value);
}
...
async function startEvaluation() {
  ...
  try {
    ...
    // 打印确认
    console.log('最后agentIds的值:', agentIds, Array.isArray(agentIds) ? 'array' : typeof agentIds);
    console.log('最后datasetId的值:', datasetId, typeof datasetId);
    ...
    // 调用后端接口,获得评估的记录
    const res = await AiGenerateEvaluation.generateEvaluationRecord(JSON.stringify(params));
    console.log('获得评估的记录:', res);
    // 把返回的record_id传给generateEvaluationRecordResult 获得评估的记录详情
    const res2 = await AiGenerateEvaluation.generateEvaluationRecordResult({ record_id: res.data.record_id })
    console.log('获得评估的记录结果分数:', res2);
